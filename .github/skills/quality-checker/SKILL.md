...
Execute comprehensive quality checks on the RFC module codebase using the same containerized environment as CI pipelines. Ensures consistency between local development and automated testing.
...
This skill runs quality checks inside the official test container used by this repository (`ghcr.io/rdkcentral/docker-device-mgt-service-test/native-platform:latest`), which includes:
...
docker exec -i native-platform /bin/bash -c "
   cd /mnt/workspace && \
   sh -e cov_build.sh && \
   for bin in rfcMgr rfcMgr/gtest/rfcMgr_gtest rfcMgr/gtest/rfcapi_gtest rfcMgr/gtest/tr181api_gtest rfcMgr/gtest/utils_gtest; do
      if [ -f \"\$bin\" ]; then
         ls -lh \"\$bin\"
         file \"\$bin\"
         size \"\$bin\"
...
**User**: "Full analysis on src"
- Run all checks scoped to the RFC module sources
...
1. **Check local image first**
   - Reuse the existing native-platform image when present
   - Pull only when needed and only with user approval outside CI
