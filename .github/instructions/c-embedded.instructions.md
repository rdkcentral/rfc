...
typedef enum {
    RFC_ERROR_SUCCESS = 0,
    RFC_ERROR_FAILURE = -1,
    RFC_ERROR_INVALID_PARAM = -2,
    RFC_ERROR_NO_MEMORY = -3,
    RFC_ERROR_TIMEOUT = -4
} RFC_ERROR;

RFC_ERROR init_rfc_manager() {
    if (!validate_config()) {
        return RFC_ERROR_INVALID_PARAM;
    }
    
    if (allocate_resources() != 0) {
        return RFC_ERROR_NO_MEMORY;
    }
    
    return RFC_ERROR_SUCCESS;
}
```
...
if (ret != 0) {
    RDK_LOG(RDK_LOG_ERROR, LOG_RFCMGR,
            "%s:%d Failed to initialize: %s (errno=%d)\n",
            __FUNCTION__, __LINE__, strerror(errno), errno);
    return RFC_ERROR_FAILURE;
}
```
...
if (ret != 0) {
    RDK_LOG(RDK_LOG_ERROR, LOG_RFCMGR, "Failed to create thread: %s\n", strerror(ret));
...
int safe_lock_with_timeout(pthread_mutex_t* lock, int timeout_ms) {
    ...
    if (ret == ETIMEDOUT) {
        RDK_LOG(RDK_LOG_WARN, LOG_RFCMGR, "Lock timeout - potential deadlock detected\n");
        return -1;
    }
    return ret;
}
 ...
 */
int process_rfc_param(const rfc_param_t* param) {
...
### Naming Conventions
- Functions: `snake_case` (e.g., `init_rfc_manager`)
- Types: `snake_case_t` (e.g., `rfc_param_t`)
...
```c
// GOOD: header guard
#ifndef RFC_INTERNAL_H
#define RFC_INTERNAL_H

// ... declarations ...

#endif /* RFC_INTERNAL_H */
```
...
- See `rfcapi/rfcapi.h` for the public RFC API header documentation
- Review existing code in `rfcMgr/` for implementation patterns
- Check `rfcMgr/gtest/` directory for testing examples
