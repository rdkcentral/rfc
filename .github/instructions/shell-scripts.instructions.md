...
if ! mkdir -p /tmp/rfc; then
    logger -t rfc "ERROR: Failed to create directory"
    exit 1
fi
...
log_info() {
    logger -t rfc -p user.info "$*"
}

log_error() {
    logger -t rfc -p user.error "$*"
    echo "ERROR: $*" >&2
}

# Usage
log_info "Starting RFC configuration fetch"
