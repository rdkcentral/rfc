...
TEST(rfcMgrTest, MAX_RETRIES) {
    ...
    int ret = rfcObj->GetBootstrapXconfUrl(boot_strap_xconf_url);
    std::cout << "boot_strap_xconf_url = " << boot_strap_xconf_url << std::endl;
    EXPECT_EQ(boot_strap_xconf_url, "");
    EXPECT_EQ(ret, -1);
    delete rfcObj;
}
...
TEST(rfcMgrTest, getUpgradeHashAndTime) {
    RuntimeFeatureControlProcessor *rfcObj = new RuntimeFeatureControlProcessor();
    rfcObj->rfc_state = Init;
    rfcObj->_last_firmware = "SKXI11ADS_MIDDLEWARE_DEV_develop_20250105135149";
    rfcObj->_partner_id = "Unknown";
    std::string hashValue;
    std::string valueTime;
    rfcObj->GetStoredHashAndTime(hashValue, valueTime);
    std::cout << "hashValue = " << hashValue << " valueTime = " << valueTime << std::endl;
    EXPECT_EQ(hashValue, "UPGRADE_HASH");
    EXPECT_EQ(valueTime, "0");
    delete rfcObj;
}

TEST(rfcMgrTest, GetRFCPartnerID) {
    writeToTr181storeFile(RFC_PARTNERNAME_KEY_STR, "sky", "/opt/secure/RFC/tr181store.ini", Quoted);
    RuntimeFeatureControlProcessor *rfcObj = new RuntimeFeatureControlProcessor();
    rfcObj->GetRFCPartnerID();
    delete rfcObj;
    EXPECT_EQ(rfcObj->_partnerId, "sky");
}
...
TEST(rfcMgrTest, GetOsClass_Disabled) {
    ...
    RuntimeFeatureControlProcessor *rfcObj = new RuntimeFeatureControlProcessor();
    rfcObj->GetOsClass();
    EXPECT_EQ(rfcObj->_osclass, "OsClass");
    delete rfcObj;
}
...
/*TEST(rfcMgrTest, Features_NotFound) {
    JSON *pJson = ParseJsonStr(xconfResp);
    if (pJson) {
        RuntimeFeatureControlProcessor *rfcObj = new RuntimeFeatureControlProcessor();
	JSON *features = rfcObj->GetRuntimeFeatureControlJSON(pJson);
	EXPECT_STREQ(features, nullptr);
	delete rfcObj;
    }
}*/
