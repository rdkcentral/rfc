...
def test_direct_block_is_active_within_24_hours():
    ...
    try:
        # RFC_LOG_FILE can rotate past LOG_FILE (.1, .2, ...) depending on log
        # volume, so clear every rotated backup to keep grep_log_file scoped
        # to just this run.
        remove_rotated_logs(RFC_LOG_FILE)
        rfc_run_binary()
