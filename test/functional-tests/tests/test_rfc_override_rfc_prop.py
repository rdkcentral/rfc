...
def test_rfc_override_dbg_srv_locked():
    ...
    try:
        ...
        # Validate stdout belonging only to this rfcMgr invocation.
        ...
        default_request_msg = (
            f"Xconf Request : [{RFC_XCONF_URL}"
        )

        # PROD must select the normal RFC URL.
        assert default_url_msg in rfc_output, (
            "RFC did not select the normal /etc/rfc.properties URL "
            "while debug services were locked"
        )

        assert default_request_msg in rfc_output, (
            "RFC did not issue the Xconf request using the normal "
            "/etc/rfc.properties URL"
        )

        # /opt/rfc.properties must not be used.
